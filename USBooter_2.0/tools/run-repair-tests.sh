...
if command -v fsck.hfsplus >/dev/null; then fsck.hfsplus -f -n /tmp/applefs-hfs.img && echo "fsck.hfsplus OK" || echo "fsck.hfsplus cannot read plain image files here"; fi
